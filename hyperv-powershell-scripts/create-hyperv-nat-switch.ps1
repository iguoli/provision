...
If ("NATSwitch" -in (Get-VMSwitch | Select-Object -ExpandProperty Name) -eq $FALSE) {
    ...
    New-NetIPAddress -IPAddress 192.168.33.10 -PrefixLength 24 -InterfaceAlias "vEthernet (NATSwitch)"
...
If ("192.168.33.10" -in (Get-NetIPAddress | Select-Object -ExpandProperty IPAddress) -eq $FALSE) {
    'Registering new IP address 192.168.33.10 on Windows Hyper-V host...'

    New-NetIPAddress -IPAddress 192.168.33.10 -PrefixLength 24 -InterfaceAlias "vEthernet (NATSwitch)"
}
else {
    '"192.168.33.10" for static IP configuration already registered; skipping'
}
